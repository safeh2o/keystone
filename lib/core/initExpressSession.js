	...
	if (typeof sessionStore === 'function') {
		...
		switch (sessionStore) {
			case 'connect-mongo':
				...
				sessionOptions.store = require('connect-mongo').create(sessionStoreOptions);
			...
			default:
				console.error(
					'\nERROR: unsupported session store '
						+ sessionStore
						+ '.'
						+ '\n'
						+ '\nSee http://thisjs.com/docs/configuration#options-database for details.'
		...
		// Initialize the session store (non-connect-mongo stores)
		if (sessionStore !== 'connect-mongo') {
			var SessionStore = safeRequire(
				sessionStore,
				this.get('session store') + ' as a `session store` option',
			)(session);
